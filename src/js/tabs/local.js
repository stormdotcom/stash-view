import { getLocalStorage, setLocalStorage, removeLocalStorage, clearLocalStorage } from '../lib/inspected-page.js';
import { detectType, formatBytes, tryParseJSON, escapeHtml } from '../lib/format.js';
import { JsonTree } from '../components/json-tree.js';

export class LocalTab {
  ...
  async load() {
    try {
      this.items = await getLocalStorage() ?? [];
    } catch (e) {
      this.items = [];
  ...
  async clearAll() { await clearLocalStorage(); }
  ...
  _showDetail(key, value) {
    ...
    // Add copy button for full value
  ...
  _showEditor(key, value) {
    ...
    saveBtn.addEventListener('click', async () => {
      await setLocalStorage(key, textarea.value);
      await this.load();
    });
